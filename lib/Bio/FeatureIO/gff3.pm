    ...
    while (my $line = $self->_readline) {
        ...
        for ($line) {
            ...
            when (/^(\#{1,2})\s*(\S+)\s*([^\n]+)?$/) { # comments and directives
                if (length($1) == 1) {
                    ...
                    @{$dataset}{qw(MODE DATA)} =
                        ('directive', Bio::GFF3::LowLevel::gff3_parse_directive($line));
                }
            }
            when (/^>/) {          # sequence
                chomp $line;
            ...
            when (/(?:\t[^\t]+){8}/)  {
                ...
                $dataset->{DATA} = Bio::GFF3::LowLevel::gff3_parse_feature($line);
            }
            default {
...
sub next_dataset_old {
    my $self = shift;

    my $unescape_func = {
        3 => \&Bio::GFF3::LowLevel::gff3_unescape,
      }->{$self->version}
      || sub { $_[0] };

    my $escape_func = {
        3 => \&Bio::GFF3::LowLevel::gff3_escape,
      }->{$self->version}
      || sub { $_[0] };
...
sub _write_feature_3 {
    ...
    my $str = Bio::GFF3::LowLevel::gff3_format_feature( $self->_gff3_lowlevel_hashref( $feature, $parent_feature ))
